...
import { completionCount, filterPracticeQuestions, gradePoints, reconcileSessionQuestions, selectBalancedQuestions, sessionScore, toggleExpandedUnitIds, toggleUnitSelection, unitSelectionState, type PracticeDifficulty, type PracticeDisplay, type PracticeQuestion, type StudySession } from './training.page';

const session = (results: StudySession['results']): Pick<StudySession, 'results'> => ({ results });

describe('Training Mode sessions', () => {
  ...
  it('removes withdrawn questions from saved training sessions', () => {
    const value: StudySession = { id: 's1', courseId: 'ap-calculus', name: 'Test', topicIds: ['1.6'], questionIds: ['withdrawn', 'kept'], currentIndex: 1, totalSeconds: 0, results: { withdrawn: 'cooked', kept: 'close' }, revealed: ['withdrawn'], status: 'open', createdAt: '2026-01-01', updatedAt: '2026-01-01' };
  ...
  it('drops a saved run when all of its questions were withdrawn', () => {
    const value: StudySession = { id: 's1', courseId: 'ap-calculus', name: 'Test', topicIds: ['1.6'], questionIds: ['withdrawn'], currentIndex: 0, totalSeconds: 0, results: {}, revealed: [], status: 'open', createdAt: '2026-01-01', updatedAt: '2026-01-01' };
