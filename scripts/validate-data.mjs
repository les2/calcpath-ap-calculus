...
for (const topic of topics.filter((topic) => /^(1|2|3)\./.test(topic.id))) {
  assert(embeddedQuestions.filter((question) => question.topicId === topic.id).length >= 5, `Unit 1, 2, or 3 topic ${topic.id} needs at least five embedded publisher-authored questions.`);
}
