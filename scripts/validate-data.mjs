...
for (const topic of topics.filter((topic) => /^(1|2|3|4|5)\./.test(topic.id))) {
  assert(embeddedQuestions.filter((question) => question.topicId === topic.id).length >= 5, `Unit 1–5 topic ${topic.id} needs at least five embedded publisher-authored questions.`);
}
